...
java -jar /picard.jar CollectGcBiasMetrics --INPUT ${sample_id}.sorted.bam --OUTPUT ${sample_id}_GCbias.txt -CHART ${sample_id}_GCbias.pdf --SUMMARY_OUTPUT ${sample_id}_GCmetrics.txt -R $params.outdir/ref/index/genome.fa 
...
java -jar /picard.jar CollectAlignmentSummaryMetrics -R $params.outdir/ref/index/genome.fa -I ${sample_id}.sorted.bam -O ${sample_id}_aln_metrics.txt 

java -jar /picard.jar BamIndexStats -I  ${sample_id}.sorted.bam > ${sample_id}_indexstat.txt

java -jar /picard.jar CreateSequenceDictionary --R $params.outdir/ref/index/genome.fa --O fasta_modified.dict 
