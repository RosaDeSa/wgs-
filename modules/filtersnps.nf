...
process filtersnps {
   ...
   input:
   tuple val(sample_id), val(id_patient), val(gender), val(id_run), path(select_snps)
   path(fai)
   path(genome_dict)
   //path(gatk_dict)
   path(known_dbsnp)
   ...
   output:
   tuple val(sample_id), val(id_patient), val(gender), val(id_run), path("${sample_id}_filtered_snps.vcf"), emit:  filtered_snps
